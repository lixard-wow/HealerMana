...
local function safeStr(v)
    ...
    if HM.isSecretValue and HM.isSecretValue(v) then return "<secret>" end
...
local function printAuraDump()
    ...
    local function reportUnit(unit)
        ...
        local slotResults = {pcall(C_UnitAuras.GetAuraSlots, unit, "HELPFUL")}
        ...
        if slotOk then
            for i = 3, #slotResults do
                local slot = slotResults[i]
                local dataOk, data = pcall(C_UnitAuras.GetAuraDataBySlot, unit, slot)
                local dataSecret = dataOk and type(data) == "table" and HM.isSecretTable and HM.isSecretTable(data)
